...
\begin{description}
    ...
    \item[Twython\footnote{\url{http://github.com/ryanmcgrath/twython}}], a thin but functional Python wrapper around the Twitter API, makes API calls easily available within Python. It also provides a set of methods that handle \todo{Add authentication section}authentication with Twitter.
\end{description}
...
\todo{Move to deployment section}Nginx\footnote{\url{www.nginx.org}} was used to proxy the requests to the client and server, as well as serving all static files. This not only significantly boosted the performance of the system, but also made it more secure and accessible.
...
The solution to this issue was to use operating system level processes in the place of threads. This gives each process a separate interpreter, and therefore a separate GIL and a separate thread of execution --- achieving true parallel processing. However, this raised a new issue: each process has its own protected memory space, which therefore makes the exchange of data between processes difficult.
...
Git\footnote{\url{www.git-scm.com}} is a distributed version control system that is both free and open source. It allows the user to create a repository that stores a complete history and full version tracking that is not dependent on network access or a central server (unlike the once popular alternative, Subversion\footnote{\url{http://subversion.apache.org}}).
