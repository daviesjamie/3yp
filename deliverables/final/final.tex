...
\textcite{Utiyama:2006} showed it is possible to combine collaborative filtering, content-based filtering and relevance feedback techniques into one system to provide better recommendations.
...
A custom extension of the Django authentication module was created to use the Twitter OAuth API authenticate users. Using the appropriate authentication methods exposed through the Twython library, users were created in Django that stored the OAuth tokens for each user after they had approved the client application to access their account. This then enabled use of the useful user library from within Django, but also stored the OAuth tokens necessary for communication with Twitter.
