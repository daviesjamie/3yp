...
This project presents an intelligent hashtag recommendation tool for use with Twitter, that makes it easier for users to compose tweets with relevant hashtags, as well as aiding the search and navigation of Twitter as a whole. The presented technique integrates a Naive Bayes classifier with a novel stream processing framework to give functional, non-personalised hashtag suggestions and search query expansions.
...
\begin{figure}[htpb]
    ...
    \caption{An example showing the tagging practices used on Twitter.}
    \label{fig:badtagging}
\end{figure}

\section{Project Goals}

The goal of this project is to develop an intelligent hashtag recommendation system for use with Twitter. The objective is to make it easier for users to select relevant hashtags for their tweets, and in doing so, begin to tackle the problems presented by the sheer quantity and redundancy of hashtags in Twitter. The aim is that the user will be able to see hashtag suggestions in real-time as they are typing a tweet, thus increasing the likelihood of them publishing tweets with accurate hashtag data.

Such a recommendation system would also need to aid users in the navigational and exploratory aspects of folksonomies. Therefore, another objective for this project is to make it easier for users to navigate and search the fragmented hashtag data already published on Twitter. The aim is that the same recommendation system used to suggest hashtags for content a user is creating could also be applied to create enhanced search queries to retrieve tweets from Twitter.
...
\begin{figure}[htpb]
    \centering
    \subfigure[Music recommendation on Last.fm]{
        \includegraphics[width=0.4\linewidth]{lastfm.png}
        \label{fig:lastfmrec}
    }
    \subfigure[Suggested purchases on Amazon]{
...
After studying the advantages of providing personalised recommendations in retail situations on a per-user basis, \textcite{Kywe:2012} realised that a similar approach towards hashtags could prove fruitful. Hashtag use varies from person to person, with some users using the latest trending hashtags, other users only using a specific set or type of tag, and with some users barely making use of them at all. They proposed a personalised hashtag recommendation system that considers both user preferences and the query tweet content: the system creates a ranked list of hashtags from both the most similar users and most similar tweets. This gave promising results, although it was noted that this may not be the best recommendation system for all types of tweets and hashtags.
...
As demonstrated above, there is a need for a hashtag recommendation system to improve the quality and clarity of hashtags in Twitter, as well as to aid navigation of the fragmented data that already exists. This leads to the two main requirements the system in this project must fulfil:
\begin{itemize}
...
This raises the question of whether hashtags are clearly defined, or whether their meanings (and therefore the tweets they apply to) are dependent upon user opinion. Through the preliminary examination of a sample Twitter dataset conducted in Section~\ref{sec:preliminaryanalysis}, this project is inclined to believe that the correct usage of a hashtag is independent of how frequently a particular user uses that hashtag. Indeed, it could be argued that a user's preference towards a particular hashtag can result in incorrect hashtags being used --- which is the very problem this project is aiming to tackle.
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}[
        ...
        % Arrows
        \draw[<->,line width=1.2pt] (a) -- (interface);
        \draw[<->,line width=1.2pt] (b) -- (interface);
        \draw[<->,line width=1.2pt] (c) -- (interface);
        \path[<->,line width=1.2pt] (clientbox) edge node[sloped, anchor=center, above] {\footnotesize HTTP} (swebserverbox);
        \draw[<->,line width=1.2pt] (clientbox) -- (twitter);
        \draw[->,dashed,line width=1.2pt] (twitter) -- (train.east);
    \end{tikzpicture}
...
To choose an appropriate classification algorithm, two main factors must be considered. One factor is that the classifier has to be capable of being trained incrementally. This is so that the classifier can be trained upon a stream, and simultaneously still provide valid classifications. This is essential, as some classifiers require parsing the entire data set before they are able to provide classifications, making them unsuitable for working with data stream. The other factor to consider is that of probabilities: it has been decided that it would be suitable for the classifier to return the probabilities of the hashtag recommendations it suggests (as seen in Functional Requirement~\ref{func:probabilities}).

These considerations have led to the choice of a Naive Bayes classifier. Naive Bayes is built upon Bayes' Theorem, which explains how to derive the probability of an event happening given that another event has occurred:
...
This chapter explains how the system has been implemented, and the decisions that were made during the process. It also shows any difficulties that were encountered, and how they were dealt with.
...
\begin{description}
    ...
    \item[Flask\footnotemark]\footnotetext{\url{http://flask.pocoo.org}} and \textbf{Tornado\footnote{\url{www.tornadoweb.org}}} are web application frameworks made with Python that make it simple to create efficient and scalable web APIs and WSGI applications. They are used to create and handle the HTTP endpoints for the server.
    \item[Twython\footnotemark]\footnotetext{\url{http://github.com/ryanmcgrath/twython}} is a thin but functional Python wrapper around the Twitter API that makes API calls easily available within Python. It also provides a set of methods that handle authentication with Twitter.
\end{description}
...
\begin{description}
    ...
    \item[JQuery\footnotemark]\footnotetext{\url{www.jquery.com}}, a Javascript library that contains a huge set of predefined functions and classes. It is designed to simplify client-side Javascript, and makes it much easier to manipulate the document object model (DOM) from within Javascript --- it is used specifically within the client to perform asynchronous Javascript and XML requests (AJAX requests) to the server, and to provide live hashtag suggestions.
\end{description}
...
It is important to note here that the dictionaries simply store the count of each feature, and can therefore be quickly and easily incremented independently of the other features. Traditionally, Naive Bayes stores the probabilities of each feature --- but doing this here would result in all of the probabilities having to be recalculated every time a new feature was processed. This would be a highly inefficient process, and so the probabilities are instead calculated on demand when needed.
...
Another issue with this approach to Naive Bayes is that when there is very little data for a given hashtag or token, the probability can become heavily biased. For example, if a token has only been seen once with a hashtag, then that token will have a 100\% probability of being used in conjunction with that hashtag. This effect was overcome with a weighting function, $P*$, that adds a small bias towards an assumed probability:
...
To successfully share the classifier object between processes, and not have each process operating on a separate instance, a proxy object is used. Extending the \texttt{BaseManager} class from the \texttt{multiprocessing}\footnote{\url{http://docs.python.org/2/library/multiprocessing.html}} library, the proxy object exposes a selection of the classifier methods to all of the processes in a virtual classifier object. The manager itself runs a server process, with which the other processes communicate via the virtual classifier object. The manager then relays the function calls upon the proxy object to a concrete instantiation of the classifier.
...
\section{Client}
The classification server is designed with an open REST API (see Appendix~\ref{appendix:restapi}), which creates the opportunity for a myriad of different clients to use the recommendation system. However, for the purposes of this project, a simple interface was created to demonstrate how the classification server can be accessed. For screenshots of the client interface, please see Appendix~\ref{appendix:screenshots}.
...
Django views for both the popular hashtags and tokens use the \verb+urllib+\footnote{\url{http://docs.python.org/2.7/library/urllib2.html}} library to fetch the relevant information from the classifier REST API, before embedding this data in a HTML page. The tag clouds were then generated using the \verb+jquery.tagcloud.js+\footnote{\url{http://github.com/addywaddy/jquery.tagcloud.js/}} JQuery plugin.

\subsection{Authentication}
Twitter uses OAuth to handle the authentication of users through third-party applications. OAuth is an open authentication protocol that allows users to approve applications to act on their behalf without sharing their password.\footnote{\url{http://dev.twitter.com/docs/auth/oauth/faq}}

A custom extension of the Django authentication module was created to use the Twitter OAuth API to authenticate users. Using the appropriate authentication methods exposed through the Twython library, users were created in Django that stored the OAuth tokens for each user after they had approved the client application to access their account. This then enabled use of the useful user library from within Django, but also stored the OAuth tokens necessary for communication with Twitter.
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}[
        ...
        \node (user) [title] {User};
            \node (django) at ($(user.west)+(0,-28pt)$) [title,xshift=2mm] {Default Django Fields};
                \node (username) [below=of django.west,typetag] {Username};
                \node (password) [below=of username.west,typetag] {Password};
            \node (custom) at ($(password.west)+(0,-32pt)$) [title] {Custom Fields};
                \node (oauth) [below=of custom.west,typetag] {OAuth Token};
                \node (oauthsecret) [below=of oauth.west,typetag] {OAuth Secret Token};

        \node (djangobox) [draw,fit={(django) (username) (password)},dashed] {};
        \node (custombox) [draw,fit={(custom) (oauth) (oauthsecret)},dashed] {};
        \node (userbox) [draw,fit={(user) (djangobox) (custombox)}] {};

        % Twitter
        \node (twitter) at (8cm, -2.5cm) [cloud,draw,cloud puffs=10,cloud puff arc=120,aspect=2,inner ysep=1em] {Twitter};

        % Arrows
        \draw[->] (twitter) -- (oauth.east);
        \draw[->] (twitter) -- (oauthsecret.east);
        \draw[->] (twitter) -- (username.east);
    \end{tikzpicture}
    \caption{The components of a client \texttt{user} object.}
    \label{fig:clientuser}
\end{figure}

\section{Deployment}
Due to the limited resources available for this project, the client and the server were both hosted on a virtual machine (VM) on the University of Southampton network. Separate instances of Tornado were used to serve the client and server on different ports to the local host only. Nginx\footnote{\url{www.nginx.org}} was then used to proxy outside requests from port 80 (the default HTTP port) to the correct ports on the local machine for the client and server. Additionally, Nginx was used to serve all static files, such as CSS or Javascript files. This approach not only significantly boosted the performance of the system, but also made it more secure and accessible.

It is intended, however, that in a real deployment scenario the client and server would be hosted on separate machines.

\begin{figure}[htpb]
    \centering
    \begin{tikzpicture}[
            every node/.style={draw,rectangle,node distance=2.8cm,inner sep=5pt,text width=1.6cm,minimum height=1.2cm,text badly centered}
    ]
        \node (classserver) {Classifier Server};
        \node [left of=classserver] (flask) {Flask REST API};
        \node [below of=flask] (django) {Django Client Interface};
        \node [left of=flask] (tornados) {Tornado};
        \node [left of=django] (tornadoc) {Tornado};
        \node (nginx) at ($(tornados.west)+(-2cm,-1.4cm)$) [minimum height=4cm] {Nginx};
        \node (twitter) at ($(nginx.west)+(-2cm,1.4cm)$) [cloud,cloud puffs=10,cloud puff arc=120,aspect=2] {Twitter};
        \node [below of=twitter,ellipse] (users) {Users};

        \draw[<->,line width=1.5pt] (classserver) -- (flask);
        \draw[<->,line width=1.5pt] (flask) -- (tornados);
        \draw[<->,line width=1.5pt] (tornados) -- ($(nginx.east)+(0,1.4cm)$);
        \draw[<->,line width=1.5pt] (django) -- (tornadoc);
        \draw[<->,line width=1.5pt] (tornadoc) -- ($(nginx.east)+(0,-1.4cm)$);
        \draw[<->,line width=1.5pt] ($(nginx.west)+(0,1.4cm)$) -- (twitter);
        \draw[<->,line width=1.5pt] ($(nginx.west)+(0,-1.4cm)$) -- (users);
    \end{tikzpicture}
    \caption{The deployment configuration of the project.}
...
Every large-scale project has to face a number of uncertain factors that could impact the progress or functionality of the final result. It is important to identify risks at the start of a project and take any steps necessary to reduce the likelihood or impact of those risks. By estimating the likelihood (a probability) of a risk occurring and the impact (on a scale of 1-4) that it would have, it is possible to calculate risk priority. This is a useful metric, as it enables realisation of the biggest threats to the project. Listed in Table~\ref{tbl:riskanalysis} are some of the risks that could have occurred during the project, the impact they would have had, and how they would have been mitigated.

\afterpage{
    \clearpage
    \begin{landscape}
        \centering
        \begin{tabular}{|L{3.5cm}|C{2.1cm}|C{2cm}|L{3.8cm}|L{6.5cm}||C{1.7cm}|}
            \hline
            \textbf{Risk} & \textbf{Likelihood ($L$)} & \textbf{Impact ($I$)} & \textbf{Consequence of Occurence} & \textbf{Mitigation Measures} & \textbf{Priority ($L\times I$)} \\
            \hline
            \hline
            Unrealistic goals & 0.6 & 4 & Unable to complete work required to fulfil original project requirements & Simplify functionality of project to ensure that a core subset of the goals and features could still be achieved, in coordination with the project supervisor & \textbf{2.4} \\
            \hline
            Unexpected workload from other University modules & 0.7 & 3 & Unable to continue following project schedule & Carefully planned schedule to balance workload. Frequently update project Gantt chart to monitor the progress and ensure project remains on track & \textbf{2.1} \\
            \hline
            Hardware failure, loss or theft & 0.3 & 4 & Loss of all completed work & Push all work in a cloud-based Git repository, therefore creating an off-site backup & \textbf{1.2} \\
            \hline
            Falling ill, or other health issues & 0.5 & 2 & Reduced productivity for an extended period of time & Follow project schedule closely, minimising the work to catch-up on. Also allow `buffer' periods in schedule to allow time for catching-up & \textbf{1.0} \\
            \hline
            Twitter API becoming unavailable or charging for access & 0.1 & 4 & Unable to use Twitter streaming, searching or authentication facilities & Use static data from an alternative source, such as the University of Southampton Twitter Archiver\footnotemark, and adjust functionality accordingly & \textbf{0.4} \\
            \hline
        \end{tabular}
        \captionof{table}{Project risk analysis, in descending order of priority.}
        \label{tbl:riskanalysis}
        \footnotetext{\url{http://tweets.soton.ac.uk/}}
    \end{landscape}
    \clearpage
}
...
\label{sec:git}
Git\footnote{\url{www.git-scm.com}} is a distributed version control system that allows the user to create a repository that stores a complete history and full version tracking. It is not dependent on network access or a central server (unlike the once popular alternative, Subversion\footnote{\url{http://subversion.apache.org}}).
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[yellow,line width=1pt,mark=+]
            coordinates {(1,30.20) (5,40.24) (10,43.54) (15,45.44) (20,47.18) (30,49.30) (50,51.80)};

            \addplot[orange,line width=1pt,mark=diamond]
            coordinates {(1,30.22) (5,40.70) (10,43.90) (15,45.78) (20,47.66) (30,49.66) (50,52.18)};

            \addplot[red,line width=1pt,mark=*]
            coordinates {(1,31.30) (5,41.10) (10,44.44) (15,46.90) (20,48.04) (30,49.96) (50,52.44)};

            \addplot[green,line width=1pt,mark=asterisk]
            coordinates {(1,30.20) (5,40.24) (10,43.54) (15,45.44) (20,47.18) (30,49.30) (50,51.80)};

            \addplot[pink,line width=1pt,mark=triangle*]
            coordinates {(1,29.76) (5,39.70) (10,43.54) (15,45.36) (20,47.10) (30,49.02) (50,51.42)};

            \addplot[violet,line width=1pt,mark=square*]
            coordinates {(1,26.56) (5,37.08) (10,40.48) (15,42.60) (20,44.42) (30,46.92) (50,49.58)};

            \addplot[cyan,line width=1pt,mark=|]
            coordinates {(1,22.66) (5,32.16) (10,35.58) (15,37.82) (20,39.54) (30,41.82) (50,44.76)};

            \addplot[gray,line width=1pt,mark=x]
            coordinates {(1,27.78) (5,37.58) (10,41.74) (15,43.86) (20,45.20) (30,47.26) (50,49.64)};

            \legend{No Filters, Hashtag Text,Stop Words, URLs, Punctuation, Usernames, All Filters,\small{Hashtag Text + Stop Words}};
        \end{axis}
    \end{tikzpicture}
    \caption{Percentage of \textbf{one or more} of $N$ recommended hashtags being correct.}
    \label{fig:onegraph}
\end{figure}
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[yellow,line width=1pt,mark=+]
            coordinates {((5,30.24) (10,35.42) (15,37.20) (20,38.94) (30,41.42) (50,43.42)};

            \addplot[orange,line width=1pt,mark=diamond]
            coordinates {(5,30.34) (10,35.66) (15,37.44) (20,39.10) (30,41.54) (50,43.70)};

            \addplot[red,line width=1pt,mark=*]
            coordinates {(5,31.58) (10,36.20) (15,38.44) (20,39.86) (30,42.04) (50,44.28)};

            \addplot[green,line width=1pt,mark=asterisk]
            coordinates {((5,30.24) (10,35.42) (15,37.20) (20,38.94) (30,41.42) (50,43.42)};

            \addplot[pink,line width=1pt,mark=triangle*]
            coordinates {(5,30.10) (10,34.98) (15,36.90) (20,38.60) (30,41.12) (50,43.08)};

            \addplot[violet,line width=1pt,mark=square*]
            coordinates {(5,27.28) (10,31.94) (15,34.22) (20,36.12) (30,38.66) (50,40.92)};

            \addplot[cyan,line width=1pt,mark=|]
            coordinates {(5,22.78) (10,27.32) (15,29.72) (20,31.12) (30,33.80) (50,36.42)};

            \addplot[gray,line width=1pt,mark=x]
            coordinates{(5,27.94) (10,33.44) (15,35.66) (20,37.14) (30,39.16) (50,41.62)};

            \legend{No Filters, Hashtag Text,Stop Words, URLs, Punctuation, Usernames, All Filters, \small{Hashtag Text + Stop Words}};
        \end{axis}
    \end{tikzpicture}
    \caption{Percentage of \textbf{all} correct hashtags being present in $N$ recommended hashtags.}
    \label{fig:allgraph}
\end{figure}
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[black,line width=1pt,mark=o]
            coordinates {(1,31.30) (5,41.10) (10,44.44) (15,46.90) (20,48.04) (30,49.96) (50,52.44)};

            \legend{\textbf{All} correct hashtags being present, \textbf{One or more} correct hashtags being present}
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{abstractclass}[text width=8cm]{Stream}{0,0}
            \operation{+ for\_each(operation : Operation, limit : int, verbose : boolean)}
            \operation{+ filter(predicate : Predicate) : FilterStream}
            \operation{+ map(function : Function) : MapStream}
            \operation[0]{+ has\_next() : boolean}
            \operation[0]{+ next() : Object}
        \end{abstractclass}

        \begin{class}[text width=5cm]{BufferedStream}{-2.8cm, -10cm}
            \inherit{Stream}
            \attribute{- buf : Queue}
            \attribute{- connected : boolean}
            \operation{+ \_\_init\_\_(buf : Queue)}
            \operation{+ connect()}
            \operation{+ disconnect()}
            \operation{+ has\_next() : boolean}
            \operation{+ next() : Object}
            \operation{+ register(item : Object)}
        \end{class}

        \begin{class}[text width=5cm]{FilterStream}{-4.4cm, -5.3cm}
            \inherit{Stream}
            \attribute{- source : Stream}
            \attribute{- predicate : Predicate}
            \operation{+ \_\_init\_\_(source : Stream, predicate : Predicate)}
            \operation{+ has\_next() : boolean}
            \operation{+ next() : Object}
        \end{class}

        \begin{class}[text width=5cm]{MapStream}{4.5cm, -5.3cm}
            \inherit{Stream}
            \attribute{- source : Stream}
            \attribute{- function : Function}
            \operation{+ \_\_init\_\_(source : Stream, function : Function)}
            \operation{+ has\_next() : boolean}
            \operation{+ next() : Object}
        \end{class}

        \begin{class}[text width=5cm]{FileInputStream}{2.8cm, -10cm}
            \inherit{Stream}
            \attribute{- input : File}
            \attribute{- next\_line: string}
            \operation{+ has\_next() : boolean}
            \operation{+ next() : Object}
        \end{class}

        \begin{abstractclass}[text width=4.7cm]{Function}{-3cm, -15.5cm}
            \operation[0]{+ apply(input : Object)}
        \end{abstractclass}

        \begin{abstractclass}[text width=4.7cm]{Predicate}{3cm, -15.5cm}
            \operation[0]{+ test(obj : Object)}
        \end{abstractclass}

        \begin{abstractclass}[text width=4.7cm]{Operation}{0cm, -18cm}
            \operation[0]{+ perform(obj : Object)}
        \end{abstractclass}
    \end{tikzpicture}
    \caption*{The \textbf{core} classes that make up the Spout library. The full code and all classes can be found at \url{http://github.com/daviesjamie/spout}.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{class}[text width=8.5cm]{Classifier}{0,0}
            \attribute{-- hashtag\_total : int}
            \attribute{-- hc : dict}
            \attribute{-- htc : dict}
            \attribute{-- lock : multiprocessing.Lock}
            \attribute{-- start\_time: int}
            \attribute{-- tc : dict}
            \attribute{-- thc : dict}
            \attribute{-- tweet\_total : int}
            \attribute{-- use\_hashtag\_text : boolean}

            \operation{+ \_\_init\_\_(use\_hashtag\_text : boolean)}
            \operation{+ classify(tweet : string, results : int) : list}
            \operation{+ get\_hashtags(num : int) : list}
            \operation{+ get\_hashtag\_tokens(num : int) : list}
            \operation{+ get\_memory\_usage() : list}
            \operation{+ get\_tokens(num : int) : list}
            \operation{+ get\_token\_hashtags(num : int) : list}
            \operation{+ get\_totals() : list}
            \operation{+ get\_unique\_counts() : list}
            \operation{+ get\_uptime() : datetime.timedelta}
            \operation{+ state\_dump(fileprefix : string)}
            \operation{+ state\_load(filename : string)}
            \operation{+ train(tweet\_tokens : list)}
            \operation{-- \_catcount(hashtag : string) : int}
            \operation{-- \_docprob(tokens : list, hashtag : string) : float}
            \operation{-- \_fcount(token : string, hashtag : string) : int}
            \operation{-- \_fprob(token : string, hashtag : string) : float}
            \operation{-- \_hashtags() : list}
            \operation{-- \_prob(tokens : list, hashtag : string) : float}
            \operation{-- \_totalcount() : int}
        \end{class}
    \end{tikzpicture}
    \caption*{The \texttt{Classifier} class, used within the server to provide all recommendations.}
\end{figure}
\pagebreak

\section{REST API Interface}
\label{appendix:restapi}
Listed in the following table are the endpoints (URLs) used for the for RESTful server API.

\renewcommand{\arraystretch}{1.5}
\begin{center}
    \begin{longtable}{|B{3cm}|M{1.5cm}|B{3.7cm}|B{3.7cm}|}
        \hline
        \textbf{Endpoint} & \textbf{Method} & \textbf{Functionality} & \textbf{Parameters} \\
        \hline
        \hline
        \endfirsthead
        \multicolumn{4}{c}{\textit{Continued from previous page}} \\
        ...
        \endfoot
        \hline
        \caption*{A list of the endpoints available through the RESTful API.}
        \endlastfoot
...
\dirtree{%
    ...
    .2 \_\_init\_\_.py. 
    .2 classifier.py. 
    .2 classifier\_manager.py. 
    .2 experiments. 
    .3 experiment\_harness.py. 
    .3 tweets.json. 
    .2 filters.py. 
    .2 oauth.json. 
    .2 oauth.py. 
    ...
    .2 tests. 
    .3 \_\_init\_\_.py. 
    .3 test\_classifier.py. 
    .3 test\_filters.py. 
    .3 test\_tokeniser.py. 
    .2 tokeniser.py. 
    .2 twokenize. 
    .3 \_\_init\_\_.py. 
    .3 emoticons.py. 
